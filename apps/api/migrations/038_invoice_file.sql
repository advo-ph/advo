...
--
-- Idempotent (2026-09-13): IF NOT EXISTS throughout, and the three CHECKs are guarded
-- ALTERs (025's form) so they are created even when `db:push` made the table first.
-- Existing databases already have them; duplicate_object makes each a no-op.
...
DO $$ BEGIN
  ALTER TABLE invoice_file ADD CONSTRAINT chk_invoice_file_phase
    CHECK (phase_status IN ('downpayment', 'full'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE invoice_file ADD CONSTRAINT chk_invoice_file_paid
    CHECK (paid_status IN ('unpaid', 'paid', 'overdue'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE invoice_file ADD CONSTRAINT chk_invoice_file_number
    CHECK (file_number >= 0);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Unique sequential number per project
CREATE UNIQUE INDEX IF NOT EXISTS idx_invoice_file_project_number
  ON invoice_file (project_id, file_number);

CREATE INDEX IF NOT EXISTS idx_invoice_file_project
  ON invoice_file (project_id);

CREATE INDEX IF NOT EXISTS idx_invoice_file_recurring
