...
function FlipCard({
  ...
  return (
    <motion.article
      className="absolute inset-x-0 top-0 aspect-[1.76/1] will-change-transform"
      style={{
        y: exitY,
        rotateX,
        opacity,
    ...
    >
      <motion.div
        className="grid h-full grid-cols-[1.12fr_0.88fr] overflow-hidden rounded-[clamp(18px,2vw,30px)] shadow-[0_16px_50px_rgba(20,17,10,0.18)]"
        style={{
      ...
      >
        <div className="flex min-w-0 flex-col p-[clamp(16px,3vw,48px)] md:pr-[clamp(22px,3vw,48px)]">
          ...
          <div className="mt-auto max-w-[46rem] pt-4 sm:pt-8">
            <p className="mb-[clamp(8px,1.5vw,22px)] text-[9px] font-semibold uppercase tracking-[0.16em] opacity-70 sm:text-xs">
              {item.eyebrow}
            </p>
            <h2 className="max-w-[16ch] text-balance text-[clamp(20px,3.25vw,48px)] font-semibold leading-[0.96] tracking-[-0.05em]">
              {item.title}
            </h2>
            <p className="mt-[clamp(8px,1.8vw,24px)] max-w-[42rem] text-[clamp(11px,1.1vw,16px)] leading-[1.5] opacity-82">
              {item.description}
            </p>
          </div>
        </div>

        <div className="relative m-[clamp(8px,1.2vw,18px)] min-h-0 overflow-hidden rounded-[clamp(12px,1.4vw,22px)] ml-0">
          <img
            src={item.image}
            alt={item.imageAlt}
    ...
    >
      <section className="relative h-[82vh] min-h-[640px] overflow-hidden px-5 sm:px-10">
        ...
        <div className="absolute inset-x-5 top-[clamp(330px,43vh,440px)] flex justify-center sm:inset-x-10">
          <h2 className="max-w-[18ch] text-center text-[clamp(42px,5.5vw,82px)] font-semibold leading-[0.92] tracking-[-0.06em] text-[#a94808]">
            {heading}
          </h2>
        </div>
      </section>
      ...
      >
        <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden px-[clamp(14px,4vw,64px)] py-8">
          <div className="relative mx-auto aspect-[1.76/1] w-full max-w-[1100px] [perspective:800px]">
