...
>
> The first-pass sources #1–32 now carry their transcripts, Drive text and case-study files. **All 468 sources have full text (0 without).** #21 and #22 (the Felici contract and proposal) got their Drive text through a direct `UPDATE corpus_source SET body` with facts untouched. Re-posting them would have deleted the 4 supersede links from #22's facts to #21's. Counts after: 34 and 22 facts, 4 links intact.
...
- ~~**`db:local` fails at `032` on a fresh DB**~~ **Resolved in `31af663`** (branch `fix/fresh-db-bootstrap`). `032`–`036` and `038` are now `IF NOT EXISTS`, and `033`'s owner backfill only runs while nobody is flagged. The CHECKs in `034`/`035`/`038`/`040` moved to guarded `ALTER`s (the `025` form), and `039`'s CHECKs are guarded. Tested on throwaway databases. A fresh `db:local` exits 0 with a clean drift check. Running the whole set again gives 0 failures and the same schema. A database built the old way (pre-`032` push plus the original migrations, with seed rows) takes the new set with no schema or row changes. Prod needs nothing: the edited files are already in its ledger and never run again.
...
- ~~**Johann Endriga and Kenneth Leo Dela Cruz have no login.**~~ **Resolved:** Kenneth (the account that owns ADVO's junior-dev sign-up form) and Johann (the account that owns the VBE project docs in Drive) now have logins — addresses on their team rows, not repeated here because this repo is public; both invite emails sent (prod mail state: 2 sent, 0 failed).
