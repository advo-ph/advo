...
`/` is the shipped `LandingPage` (`apps/web/src/components/landing/LandingPage.tsx` + `landing-page.css`). White-canvas marketing page in the runway.com grammar: fixed blur nav (transparent over the full-bleed hero on phones), cinematic hero ("We digitalize it for you.") with staggered copy and a scroll drift, a client-logo strip driven by the portfolio table, the three product surfaces on real stills, an Inquiry-to-Launch stepper over a tabbed process panel, a dark mission band carrying the four engagement shapes, the shipped-sites grid (each card is the product's own 1200x630 share frame over two inner-page screenshots from `public/work/<slug>/`, keyed in `data/work-media.ts`, and opens `/work/:slug` when a case study exists), FAQ, off-black footer. No illustrations or generated icons. The native scrollbar is hidden and `LandingScrollbar` draws a draggable overlay thumb (the sisia system); the footer floor is the traced wordmark (`public/advo-wordmark.svg`) through a CSS mask; the client strip is an infinite marquee driven by the portfolio table. Routed from `pages/Index.tsx`. Section list lives in [README.md](../README.md).
...
Live GitHub commits merged with admin-posted progress updates. Supports branch switching. A project's repository name may be owner-qualified (`CelestialBrain/vbeeyecenter`) for source that lives outside the `advo-ph` org; a bare name resolves to `GITHUB_ORG`. Webhook events match a project by the repository's full name first, then its bare name. `POST /api/github/repos/:name/backfill` (team) reads the last 100 commits on the default branch from the GitHub API and stores the ones the webhook never saw, dated by commit, so a repository linked after the fact has a history. Clients can read branches.
...
Full CRUD. Form includes: client, title, description, GitHub repo, preview URL, contract URL, status, value/paid, list value + discount + reason (migration 028; list − discount must equal the value, checked by the form, the API and a DB CHECK; blank list value means no discount), tech stack. Edit mode shows asset upload. Auto-notifies client on project status change. Each card has an **Open** button → the Project Command Center (below).
...
**Commission split** (table from migration `018`; defaults set by `037` and corrected by `045` to match the signed internal commission agreement, `data/corpus/document/internal-commission-agreement.json`): on project completion a plan allocates the project basis into **55% developer / 35% staff / 10% company reserve**, with the staff pool sub-split **20% Lead Partnerships (`referral_bps`) / 50% Management / 20% Marketing / 10% Accounting**. Prince's original 2026-06-19 structure (60/25/15, staff 28/24/24/24) was the `018` default and is superseded. Existing plans keep the split they were drafted under.
...
**Full text (migration 047).** Every source keeps the document it was extracted from in `corpus_source.body`: `/ingest/json` takes `source.body` (up to 2,000,000 characters; a re-ingest without one keeps the stored body; an old bundle's `meta.body` is moved into the column), `/ingest/text` stores the pasted text, `/ingest/plaud` stores the transcript. `GET /api/corpus/source/:id` returns `body` with the source's facts, terms and actions; `GET /api/corpus/source` is paged and returns `{ source, totalCount, limit, offset }`. `limit` runs 1–500 and defaults to 50; `offset` must be ≥ 0; anything out of range gets a 400. The order is total (occurred date, then id), so pages never skip or repeat a row. Rows never include `body`, only `body_character_count`. `?q=` searches title, summary and full text as a case-insensitive substring, where `%` and `_` match literally. `kind`, `projectId` and `externalId` filter, and every filter applies to both the page and `totalCount`. nginx allows 16m request bodies on `/api/corpus/ingest/` only; Hono sets no limit.

**Document view.** On the Corpus screen's Sources tab, a search box queries title, summary and full text, with kind and project filters beside it. The header reads "N of total", and a "Load more" button fetches the next 50 until every source is shown. Each row shows its character count or "no full text". Clicking a source opens it in place (`/admin/corpus?source=<id>`, so reload and links work; Back returns to the list): title and external id, a link to the original, kind, project, occurred and ingested dates, the summary, then the full body in a bordered box that scrolls on its own (at most 70% of the viewport, wrapping, no horizontal overflow at 375px) with a Copy button, then the facts, terms and actions taken from it. There is no markdown renderer in the repo, so markdown is shown as wrapped monospace source; plain transcripts use the body font.

**Fact-check.** `POST /api/corpus/check { claim }` searches claim + quote by words (numbers are deliberately not search terms), then compares the numbers in the claim against the numbers in each match and answers supported / conflicting / unknown with the matches, quotes, timestamps and sources shown. A superseded fact never counts as support: if only a superseded fact agrees, the verdict is conflicting and the match is shown marked. The any-word fallback keeps only matches that name what the claim names (a client, a product, a person). When the words match but no match carries the claim's figure (four digits or more), the figure is searched as text, digits with optional thousands separators on their own digit boundary, narrowed to the names the claim uses, and those facts are put first, so a true figure phrased without the client's name is not called a conflict. `isContested` is set when a strong live match carries a different number. It is a comparison, not an oracle, and the screen says so.

**Supersession.** `POST /api/corpus/supersede` reads every typed term across contract, proposal and addendum sources, groups them by project and term name, newest document first, and points every fact the older document stated at the older figure (cents terms match either spelling, `300000` or `₱3,000`) to the newest document's fact carrying the newer figure. Money and commitment facts spoken on recordings dated before the newer document, on the same project, are superseded the same way. Idempotent; returns the pairs it acted on. Terms whose digits overlap (a 15-day window in both) are left alone.

**Discounts.** A discount is a fact about a price, not a new price. Any ingested text is read for one deterministically, whatever the extractor made of the sentences: `discount_pct` or `discount_cents`, `discount_reason` ("referral", or the clause after "because") and `list_fee_cents` (the figure the discount is taken from) land as terms. Supersession leaves an older document's money terms alone when the newer one carries a discount, so a contract's list price stays live beside the addendum that discounted it. Fact-check answers a figure no sentence wrote down when a list price and a discount explain it ("₱300,000 less 10% = ₱270,000, per Bench arith proposal") and returns the arithmetic as `discount`. `DELETE /api/corpus/source/:id` (admin) removes a bad ingest; facts elsewhere that pointed at its facts as successors go live again.

**Verification pass.** 825 facts are only useful if the pricing and terms can be trusted. `GET /api/corpus/fact?verified=false&category=pricing` is the review queue; `POST /api/corpus/fact/verify` marks many facts at once. The Corpus screen has an "Unverified only" toggle and a "Verify all shown" button behind a confirm, so a reviewer works a queue.

**Proposal.** `POST /api/corpus/proposal { projectId, corpusTemplateId? }` assembles a project's current numbers into a draft, and refuses when the sources disagree. A term is taken from the source that still asserts it (a non-superseded fact carries its figure, or a pure-term source at face value); two live values for one term is a contest, listed with both sources, and no draft is produced until supersession picks the newer document. The discount on the project row is applied, the chosen template is filled (a cents term gets a peso alias, `total_fee_cents` fills `{{total_fee}}`), unfilled placeholders are reported, and each resolved value cites its source. The Corpus screen's Draft tab drives it. `DELETE /api/corpus/template/:id` and `DELETE /api/corpus/source/:id` (admin) remove a template or a bad ingest.

**Repository knowledge.** `npm run corpus:repo` reads the case studies in `apps/web/src/data/case-study.ts` (each feature cites the file in the client repository that proves it) and writes one bundle per repository under `data/corpus/repo/`, kind `local_file`, external id `advo-ph/<repo>`: a fact per feature with the proof path as locator, plus stack and integration facts and a `feature_count` term. So the corpus knows what ADVO shipped, not only what it promised.

**Loader.** `npm run corpus:load` posts every bundle under `data/corpus/meeting`, `data/corpus/document` and `data/corpus/repo` (`--only meeting|document|repo|template`, `--dry-run`), plus `data/corpus/document/TEMPLATE.json` and `data/corpus/template/*.json`, to an API (`ADVO_API_URL`, `ADVO_EMAIL`, `ADVO_PASSWORD`). Idempotent; exit 1 if anything was skipped. A bundle that names a project id the target database does not have is loaded with the id dropped and remembered in `meta.unresolvedProjectId`. A `leadName` the pipeline has never seen becomes a `lead` row.
...
| [LEGAL-BRIEF.md](LEGAL-BRIEF.md)                                             | The packet for counsel. All nine policies with their operative language quoted inline, 52 closed-form questions, the RA 10173 outreach block, real commercial figures, a TODO annex for entity details, and the bounded engagement ask. Sendable as-is; graded by `npm run bench:legal` (9/9). |
