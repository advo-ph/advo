...
Three of these need nobody but you.
...
**What it unblocks:** "Show Client Now" deploying a real preview instead of the team pasting a URL.
A live call currently returns Cloudflare error `9106: Authentication failed`, which proves the
endpoint works and the credential does not.
...
copy. Confirmed missing 2026-08-28: `GET /api/health` reports
...
## 10. `GITHUB_TOKEN` / `GITHUB_WEBHOOK_SECRET` — **state unknown**

**Who can get it: you.** But first find out whether it is already set: health does not report it, so
this is the one item on this list I could not check from outside the box.
...
1. **Cloudflare token** (1) and **`ANTHROPIC_API_KEY`** (2) — self-serve, minutes, immediate
   capability. No reason to wait on either.
2. **Apex SPF, Workspace DKIM, DMARC `rua=`** (3) — improves every email already being sent.
3. **Send the identity ask to Prince** (7) and **the brief to counsel** (8) — both carry long human
   latency, so start the clock now, in parallel with everything else.
4. **DTI / BIR documents → PayMongo** (6) — 14 business days, and it needs 7 done first.
