...
| **Latest platform work (2026-06-20)** | Project Command Center (per-project hub) · contract red-flag review (now **AI-capable**, Claude + heuristic fallback) · Files/Drive pillar · Show-Client-Now expiring preview links + client request-from-Hub · email-on-new-lead · Deliverables CRUD · Tier-1/2 security fixes (incl. **S4 closed**) + DB FK migration · **Linear UI redesign** (admin + hub — Hanken Grotesk, dense tables, no mono) · **ADVO records calendar** — Phase 1 (month grid + derived deliverable/invoice/project events + manual event CRUD) **live**; **Phase 2** (content/social + contracts/MOA + BIR-deadline layers derive into the union; new `contract` table + CRUD at `/api/contracts`, migration `004`; BIR deadlines auto-generated, no table) **deployed to prod 2026-06-20** (migration `004` applied as app DB user, web rsynced, `advo-api` restarted; BIR ruleset still needs accountant validation). Phase 3 (Google/ICS sync) decided as **two-way**. See [HANDOFF.md](HANDOFF.md). |
